...
1. Create the Xcode project: host app target + keyboard extension target, `com.<name>.*` bundle IDs (ADR-008), `RequestsOpenAccess = true` in the extension Info.plist (C-06).
2. Deploy to the iPhone on the free personal team: enable Developer Mode (C-26), trust the profile, set up Wi-Fi deployment (C-29).
3. **🔬 EMPIRICAL TEST 1 — App Groups on a free team (Q-01).** Add the capability to both targets; with Full Access on, **write from the extension, read from the host app.** Record the verdict in CONSTRAINTS.md and flip C-25's verified flag. **If it fails → buy the $99 program that day** (ADR-004); the clipboard architecture depends on it.
4. **🔬 EMPIRICAL TEST 2 — KeyboardKit 10 free-tier spike (ADR-003, Q-02).** Render a keyboard; express the Gboard bottom row `[?123][,][space][.][return]` and ~1.5× shift/backspace in the free layout engine; run **with the device in Airplane Mode** to confirm no license/network validation misbehaves inside the extension; measure baseline `phys_footprint`.
5. **🔬 EMPIRICAL TEST 3 — pasteboard behavior (Q-05).** With Full Access on, read UIPasteboard from the extension. Verify "Paste from Other Apps = Allow" on the host app suppresses the prompt (C-15), and confirm `changeCount` polling is prompt-free (C-16). Note what the alert says if it appears.
