...
| | |
|---|---|
| **Phase** | **M0 — Foundations & feasibility spike** (in progress) |
...
| Bundle IDs | `com.srimi.keyboardproject` / `com.srimi.keyboardproject.keyboard` — **never** prefixes starting `se.` or `mn.` ([ADR-008](docs/DECISIONS.md)) |
| App Group ID | `group.com.srimi.keyboardproject` — defined once in `Sources/Shared/AppGroup.swift`, never typed elsewhere |
